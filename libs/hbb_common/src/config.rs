...
pub const RENDEZVOUS_SERVERS: &[&str] = &["rd.remohelppro.jp"];
pub const RS_PUB_KEY: &str = "4OcCv+gEHKeEQcVYLn9hlO6QJtniOQ3cLuytOUq0BF8=";
    ...
    fn get_auto_id() -> Option<String> {
        ...
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        {
            ...
            if let Ok(Some(ma)) = mac_address::get_mac_address() {
                ...
                // 🔴🔴 番号が作れないときに諦めない（2026-08-15）。
                //
                //   接続番号は LANカードの MAC アドレスから作っている。
                //   取れないと元は `None` を返しており、そのとき番号は**空のまま**になる。
                //   空の番号では中継サーバーが登録を受け付けないので、
                //   ⚠ **そのアプリは永久に使えない**。画面には
                //     「接続できません。ネットワーク接続を確認してください」としか出ず、
                //     通信のせいに見えるので、原因に永久にたどり着けない。
                //
                //   ★Android/iOS は取れないとき乱数で作っている（上の分岐）。
                //     PC 版にだけ、その逃げ道が無かった。同じ逃げ道を用意する。
                //
                //   ⚠ 範囲は 0x1FFFFFFF（536,870,911）以下にする。
                //     これを超えると、常駐の目印に使っているビット（0x2000_0000）と
                //     ぶつかり、**ワンタイムの端末が常駐として扱われる**
