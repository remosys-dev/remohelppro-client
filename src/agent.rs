...
mod imp {
    ...
    pub async fn run() {
        ...
        // 🔴🔴 **残っているワンタイムを片付ける**（2026-09-23 社長のご指示）。
        //
        //   ⚠ 接続番号はMAC由来なので、ワンタイムが残っていると**同じ番号**で
        //     受付に登録され、相談員が常駐に繋いでも**そちらに届く**。
        //     ＝「パスワードが間違っています」。実機で2回起きた。
        //   ⚠ 組になっていないワンタイム（押し間違いで起動した2本目など）は、
        //     終了を知らせる相手がいないので**自分では終われない**。だからここで片付ける。
        //   ⚠ 目印ファイルのあるフォルダだけを狙う（他製品・自分は触らない）。
        #[cfg(windows)]
        crate::common::rl_kill_leftover_onetime_for_resident();
        ...
        loop {
            ...
            if let Some(token) = agent_token() {
                ...
                let mut body = json!({
                    ...
                    "appVersion": crate::VERSION,
                    // 🔴🔴 いま繋がれているかを毎回名乗る（2026-09-06 ご指摘）。
                    //
                    //   操作履歴には「接続を開始」しか無く、**いつ終わったかが
                    //   どこにも残っていなかった**（本番の実データで
                    //   開始423件・終了0件）。いつまで入っていたか示せない記録では、
                    //   無人アクセスの説明責任を果たせない。
                    //
                    //   ⚠ 終わりを知っているのは端末だけ。サーバーからは
                    //     「繋いだ」しか見えない（相談員がボタンを押した記録）。
                    //     ビュアーを閉じたことも、回線が切れたことも届かない。
                    //   ★7秒ごとのこの合図に真偽値を1つ足すだけで、
                    //     サーバー側は**切り替わった瞬間**に開始と終了を書ける。
                    //
                    //   ⚠ 画面を見ている接続だけを数える。ファイル送受信・
                    //     ポート転送・カメラは「操作した」とは別物なので混ぜない。
                    //   ⚠ lock().unwrap() は使わない。ここは 7秒ごとに回り続ける
                    //     常駐の心臓部で、一度 panic すると**合図が永久に止まる**。
                    //     取れなければ false（分からないときは書かない側に倒す）。
                    "inSession": crate::server::AUTHED_CONNS
                        .lock()
                        .map(|c| {
                            c.iter()
                                .any(|x| x.conn_type == crate::server::AuthConnType::Remote)
                        })
                        .unwrap_or(false),
                });
