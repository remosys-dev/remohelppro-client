...
pub fn core_main() -> Option<Vec<String>> {
    ...
    // 🔴🔴 **お客様用アプリを二重に起動させない**（2026-09-23 社長が実際に踏んだ）。
    //
    //   ⚠ ダウンロードの一覧から古い行（お客様用アプリ）を押し間違えるだけで、
    //     同じアプリが**2本**立ち上がる。実際に立ち上がった。
    //   ⚠ 2本になると:
    //     ・接続番号（MAC由来）を**奪い合う**。相談員が繋いでも別の方に届く
    //     ・お客様には同じ窓が2つ見え、どちらを閉じてよいか分からない
    //     ・遠隔からは「終了する」を押せないので、閉じる道が無くなる
    //   ★2本目は**立ち上げずに終わる**。
    //   ⚠ 黙って終わらない。⚠ **何も起きないと、お客様は何度も押す**。
    //   ⚠ 裏方（--server / --cm / --tray）は同じ実行ファイルの別の役なので、
    //     ここは**画面の窓（引数なし）だけ**に効かせる。
    //   ⚠ 常駐・相談員版には効かせない（名前の場所が製品ごとに分かれている）。
    #[cfg(windows)]
    if args.is_empty() && !config::IS_RESIDENT_BUILD && !config::IS_OPERATOR_BUILD {
        if crate::platform::windows::rl_another_instance_running() {
            log::info!("RL: 同じアプリが既に動いているため、2本目は起動しません");
            Toast::new(Toast::POWERSHELL_APP_ID)
                .title(&config::APP_NAME.read().unwrap())
                .text1("すでに起動しています。開いている画面をご覧ください。")
