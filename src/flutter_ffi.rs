...
pub fn main_get_app_name() -> String {
    // REMOHELP PRO rebrand (Phase 1, 2026-05-15): hard-code app name for full hide
    "REMOHELP PRO".to_string()
}

pub fn main_get_app_name_sync() -> SyncReturn<String> {
    // REMOHELP PRO rebrand (Phase 1, 2026-05-15): hard-code app name for full hide
    SyncReturn("REMOHELP PRO".to_string())
}
...
// RL build-16 (D/D2): ワンタイム版終了直前に全接続へ close_reason を送り graceful close。
//   操作側は CloseReason 受信で「お客様が接続を終了しました」を表示し再試行せず終了 → 10054 を防ぐ。
//   Bridge codegen により Dart 側で bind.mainCloseAllConnections() として呼べる。
//   呼び出し元はワンタイム版(kRlSupportShowWindow)のみ。フリートでは呼ばれず無害。
pub fn main_close_all_connections() {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    crate::server::CLIENT_SERVER
        .write()
        .unwrap()
        .close_connections_with_reason("お客様が接続を終了しました");
}
