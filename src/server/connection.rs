                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::SwitchSidesRequest(s)) => {
                        if let Ok(uuid) = uuid::Uuid::from_slice(&s.uuid.to_vec()[..]) {
                            ...
                            //     見つからないのかの区別すらできない。
                            // 🔴🔴 **どの番号へ繋ぎに行ったのかを必ず残す**（2026-09-02）。
                            //
                            //   ⚠ 9/1〜9/2、「ビュアーは起動するがオフラインで繋がらない」
                            //     という不具合を一晩追ったが、⚠ **顧客PCにも中継サーバーにも
                            //     「どの番号へ行ったか」がどこにも残っていなかった。**
                            //     中継サーバー本体は問い合わせを記録しない作り（実測で確認）。
                            //     ＝ 相談員の画面の番号を人に読んでもらう以外に手が無かった。
                            //   ★ここが唯一「相談員が名乗った番号」を知っている場所。
                            //     ⚠ 1行残すだけで、次からは記録を見れば即座に分かる。
                            log::info!(
                                "RL switch sides: お客様の側から相談員 {} へ繋ぎに行きます\
                                 （ここがオフラインと言われる場合、この番号が\
                                 中継サーバーに登録されている番号と違う）",
                                self.lr.my_id
                            );
