...
pub fn is_custom_client() -> bool {
    // 完全リネーム② (-24): APP_NAME を REMOHELP PROLink に改名すると本来ここが true 化し、
    // 焼込み custom client config を前提とする install/service 起動パスへ切替わる。
    // その config を用意していないため -23 ではサービスが起動できなかった。
    // 識別子(フォルダ/サービス/レジストリ/URI)は APP_NAME 由来でそのまま改名されるので、
    // 挙動は通常の RustDesk と同一に保つため is_custom_client は false 固定にする。
    false
}
...
pub fn is_udp_disabled() -> bool {
    // REMOHELP PRO build-24: UDP既定有効に戻す。build-23でTCP既定(!= "N")にした結果、
    //   RustDeskのID登録はUDP前提のため hbbs 1.1.15 に登録できず顧客が常にオフラインになった。
    //   UDP遮断環境はリレー(ALWAYS_USE_RELAY=Y)／将来のWS443で対応し、登録はUDPに戻す。
