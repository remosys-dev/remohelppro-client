...
impl Server {
    ...
    // RL build-16 (D/D2): ワンタイム版の正規終了時に、操作側へ close_reason を送ってから閉じる。
    //   操作側(io_loop.rs)は CloseReason 受信で msgbox 表示 + 再試行なしで終了する。
    //   → 操作側の 10054(RST/再試行) を防ぎ、かつ「お客様が接続を終了しました」と区別表示できる。
    pub fn close_connections_with_reason(&mut self, reason: &str) {
        let conn_inners: Vec<_> = self.connections.values_mut().collect();
        for c in conn_inners {
            let mut misc = Misc::new();
...
pub async fn start_server(is_server: bool, no_server: bool) {
    ...
    if is_server {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            // 🔴🔴 常駐の接続番号が「旧式」なら、ここで作り直す（2026-08-07 ご指摘）。
            //
            //   接続番号は MAC アドレスから作られるので、ワンタイム版と常駐版で
            //   **同じ番号になる**。ところがアプリ名を分けた結果、鍵は別々になった。
            //   ＝ 中継サーバーから見ると「同じ番号なのに鍵が違う相手」が2ついる。
            //   登録簿は番号1つに1行しかないので、後から登録した方が前を追い出す。
            //   実機で証明済み（本番の登録簿で鍵が入れ替わっていた）。
            //
            //   新しいビルドは、番号を作るときに30ビット目を立てて分けている。
            //   ★しかし番号が作られるのは「設定に番号が無いとき」だけ。
            //     既に入っている常駐は**古い番号を持ったまま**なので、
            //     設定フォルダを手で消してもらう必要があった。
            //     それを毎回お願いするのは現実的でない（消し忘れれば元の症状に戻る）。
            //
            //   ここで自分で直す。1台につき一度きりで、次からは何も起きない。
            //
            //   ⚠ 設定フォルダごと消す方法は採らない。消すと**固定パスワードと
            //     登録トークンまで失われ**、更新のたびに端末が登録し直しになる。
            //   ⚠ 中継サーバーへ名乗る前に直す（この下の start_all より前）。
            //     後だと古い番号で登録してしまい、また取り合いになる。
            //   ⚠ 番号が変わると、当社の台帳には**古い行が残る**。
            //     自動では消さない（通信が切れているだけの端末を消すと、
            //     戻ってきたときに承認からやり直しになるため）。「在線」に
            //     「連絡なし」と出るので、管理者が見て消せる。
            if crate::agent::is_resident() {
                const RESIDENT_ID_BIT: u32 = 0x2000_0000;
                let cur = hbb_common::config::Config::get_id();
                match cur.parse::<u32>() {
                    Ok(n) if n & RESIDENT_ID_BIT == 0 => {
                        let fixed = n | RESIDENT_ID_BIT;
                        log::info!(
                            "RL: 常駐の接続番号が旧式でした。{} → {} に作り直します\
                             （ワンタイム版と同じ番号だと、中継サーバーで取り合いになるため）",
                            n,
                            fixed
                        );
                        hbb_common::config::Config::set_id(&fixed.to_string());
                    }
                    Ok(_) => log::info!("RL: 常駐の接続番号は分離済みです ({})", cur),
                    // 数字として読めない番号（利用者が手で付けた等）は触らない。
                    Err(_) => log::info!("RL: 常駐の接続番号は数字でないため触りません ({})", cur),
                }
                hbb_common::tokio::spawn(crate::agent::run());
            }

            // 🔴🔴 相談員版とワンタイム版の接続番号も分ける（2026-08-27 実顧客で確定）。
            //
            //   ⚠ 番号は MAC アドレスから作られる（`get_auto_id()`・`id &= 0x1FFFFFFF`）。
            //     ＝ **アプリ名を分けても、番号は同じ値になる。**
            //     常駐が 0x2000_0000 を足しているのは、そのため（2026-08-07）。
            //
            //   ところが相談員版・ワンタイム版・**名前を分ける前(〜8/7)の古い版**は、
            //   3つとも番号を分けていなかった。実顧客2台で起きたこと:
            //     ・古い `C:\Program Files\remohelppro\remohelppro.exe --service` が
            //       ログイン前に起動して**先に同じ番号を登録する**
            //     ・相談員のビュアーは番号が生き返った瞬間に繋ぐので、必ず古い方に当たる
            //     ・古い方の合言葉は別物なので **「パスワードが間違っています」**
            //     ・やり直すと、そのときワンタイム版が握っていれば通る
            //   ＝ ⚠ **再起動のたびに必ず出て、再試行すると通る**という当たり外れ。
            //   実機の番号で裏づけ済み（1060458689−523587777＝ちょうど 0x2000_0000）。
            //
            //   ★上のビットを製品ごとに立てれば、範囲が重ならない。
            //     番号は 0x1FFF_FFFF 以下なので、上位2ビットで4つに分けられる。
            //   ⚠ 既存端末は設定に番号を保存済みなので、**ここで作り直す**。
            //     お客様に設定を消していただく必要は無い（常駐と同じやり方）。
            //   ⚠ 中継サーバーへ名乗る前に直す（この下の start_all より前）。
            //     後だと古い番号で登録してしまい、また取り合いになる。
            ...
            //     **もう衝突しない**。お客様のPCに手を入れなくてよくなる。
            {
                ...
                if !crate::agent::is_resident() {
                    ...
                    match cur.parse::<u32>() {
                        ...
                        Ok(_) => log::info!("RL: {}の接続番号は分離済みです ({})", who, cur),
                        // 数字として読めない番号（利用者が手で付けた等）は触らない。
                        Err(_) => {
                            log::info!("RL: {}の接続番号は数字でないため触りません ({})", who, cur)
                        }
                    }
                }
            }
