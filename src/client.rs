...
impl LoginConfigHandler {
    ...
    fn create_login_msg(
        ...
        // v1.4.6-13: Phase 4 N3 - rustdesk:// URL 経由のセッションごとに operator 情報を上書き
        // Web UI から `?peer_name=鈴木 一郎|IT 部|REMOHELP PRO Demo Org` で渡された値を
        // Flutter 側で main_set_env("RL_PEER_NAME", ...) して、Rust 側で最優先で採用する。
        // 環境変数を使うので 1 プロセス 1 セッション限定 (Phase 4 MVP 範囲)。
        let mut display_name = std::env::var("RL_PEER_NAME").unwrap_or_default();
