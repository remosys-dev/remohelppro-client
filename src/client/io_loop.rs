            ...
            match msg_in.union {
                ...
                Some(message::Union::Misc(misc)) => match misc.union {
                    ...
                    Some(misc::Union::CloseReason(c)) => {
                        ...
                        // 🔴 セッションの切り替えは「終了」ではない（2026-08-09 実機で判明）。
                        //   お客様が Windows にログインすると、被操作側のサービスが
                        //   --server を立て直すため接続が切れる。サポートは続いている。
                        //   ここを終了として扱うと、相談員の画面が
                        //   「サポートを終了しました」と出て2秒で閉じる。
                        //   実際に「起動後の最初の1〜2回だけ落ちる」という症状になっていた。
                        //   ★別種として渡し、ビュアー側で繋ぎ直させる。
                        if c == "RL_SESSION_SWITCH" {
                            self.handler.msgbox(
                                "session-switching",
                                "つなぎ直しています",
                                "お客様がパソコンにログインしました。\n自動でつなぎ直しています。",
