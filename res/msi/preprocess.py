...
def make_parser():
    ...
    parser.add_argument(
        "--app-name", type=str, default="remohelppro", help="The app name."
    )
    parser.add_argument(
        "--bin-name", type=str, default="rustdesk",
        help="The Windows binary name (no extension). Phase 3 で 'remohelppro' に変更予定だが、Cargo + Flutter ビルド設定の整合性が必要なため一旦 'rustdesk' で保留。"
    )
    parser.add_argument(
        "--product-lower", type=str, default="remohelppro",
        help="The lowercase product id for URI protocol and registry keys. アプリ本体は app_name(=remohelppro) 由来の remohelppro:// スキームを使う。MSIも同じ remohelppro:// を登録しないと、rustdesk:// を登録する他製品(stock RustDesk 等)と混線しワンクリックが別アプリを開く。/op も remohelppro:// を送る。"
    )
    parser.add_argument(
...
def gen_pre_vars(args, dist_dir):
    def func(lines, index_start):
        ...
        to_insert_lines = [
            ...
            f'{indent}<?define ProductLower="{args.product_lower}" ?>\n',
            f'{indent}<?define BinName="{args.bin_name}" ?>\n',
...
def gen_custom_ARPSYSTEMCOMPONENT_True(args, dist_dir):
    def func(lines, index_start):
        ...
        lines_new.append(
            f'{indent}<RegistryValue Type="string" Name="DisplayIcon" Value="[INSTALLFOLDER_INNER]{args.bin_name}.exe" />\n'
        )
        lines_new.append(
...
def init_global_vars(dist_dir, app_name, args):
    # v1.4.6-8 fix: dist_app は app_name (= "remohelppro" スペース入り) ではなく
    # bin_name (= "rustdesk") を使う。前者だと "remohelppro.exe" がファイル無く
    # --version 取得失敗 → init_global_vars が False → exit(-1) → Includes.wxi 注入されず
    # → WiX 'Undefined preprocessor variable $(var.Product)' エラーの真因 (v1.4.6-4/5/6/7)
    bin_name = getattr(args, "bin_name", "rustdesk")
    dist_app = dist_dir.joinpath(bin_name + ".exe")
    ...
    def read_process_output(args):
        ...
        # v1.4.6-7 fix: rustdesk.exe --version の出力が非 UTF-8 (Windows cp932 等) でも落ちないように
        # errors="replace" にする。decode 失敗で sys.exit(-1) → Includes.wxi に var.Product 注入されず
        # → WiX で "Undefined preprocessor variable" エラーになっていた v1.4.6-4/5/6 失敗の真因
