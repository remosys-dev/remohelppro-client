...
Future<List<TToggleMenu>> toolbarCursor(
  ...
  // show remote cursor
  // 🔴🔴 **一度消して、同じ日に戻した**（2026-09-04）。★経緯を残す。
  //
  //   ⚠ 私は「当社の使い方では効かない」と判断して消した。⚠ **誤りだった。**
  //     根拠にしたのは、お客様側 `server/connection.rs` の
  //       subscribe(カーソル, 相談員が操作できる || この設定)
  //     という1行だけ。⚠ **これは「送るかどうか」しか決めていない。**
  //   ★実際に絵を描くかどうかは⚠ **相談員側**が決めている:
  //       `desktop/pages/remote_page.dart`
  //       `_showRemoteCursor.isFalse ? Offstage() : CursorPaint(...)`
  //     ＝ この設定を切ると、⚠ **お客様のカーソルは描かれない。**
  //   ⚠ つまり効いていた。私が**片側だけ見て結論を出した**。
  //
  //   ★この機能の意味（社長のご指摘で正しく分かった）:
  //     ⚠ **お客様がマウスを動かすと、相談員の画面でもその位置が見える。**
  //     「そこをクリックしてください」「いま触っているのはこれです」を
  //     電話で確かめ合うのに要る。⚠ **必要な機能。**
  //
  //   ⚠ 教訓: 送る側と描く側の**両方**を見るまで「効かない」と言わない。
  //     [[check-blast-radius-before-fixing]]
