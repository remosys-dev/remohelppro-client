      ...
      if (index != -1) {
        ...
        if (client.incomingVoiceCall) {
          if (isAndroid) {
            ...
            // 音声通話の承諾は、お客様だけが押す（2026-09-04 ご判断）。
            //
            //   それまでは「相談員が遠隔で承諾を押せる状態を保つ」ために、
            //   窓を出し続ける工夫を重ねていた（出す・引っ込めさせない・
            //   前に出す・大きさを変える…）。それが不具合の温床になり、
            //   2026-09-04 には私が入れた大きさの変更でチャットまで壊した。
            //   元の素直な形に戻す: 承諾はお客様が押す。相談員は押さない。
            //   ＝ こちらは「窓を出す」だけでよい。工夫を積まない。
            //
            //   ただし出すことは必要。ワンタイム版の窓は透明にして
            //   隠してあるので、出さなければ承諾の釦がどこにも現れない。
            //   札（cmKeepVisible）は立てる。立てないと毎秒の見張りに
            //   引っ込められる（チャットで実際に起きた形）。
            cmKeepVisible = true;
            Future.delayed(Duration.zero, () async {
