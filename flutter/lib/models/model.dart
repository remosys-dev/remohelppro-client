...
class FfiModel with ChangeNotifier {
  ...
  showMsgBox(SessionID sessionId, String type, String title, String text,
    ...
    if ((title == 'Connection Error' ||
      ...
      //   ビュアーが勝手にサポートを終わらせる。
      // 🔴 切れた直後は**赤いエラーを出さない**（2026-08-02 実機指摘）。
      //
      //   お客様のPCが再起動しても、こちらが「居なくなった」と判断するまで
      //   45秒かかる（顧客アプリの生存確認が4秒ごとなので、少し余裕を見ている）。
      //   その間はサーバーが alive と答えるため、通常のエラー画面に落ちて
      //   **「接続エラー」という赤い表示**が出ていた。
      //   実際には繋ぎ直せる場面なのに、相談員には失敗にしか見えない。
      //
      //   ★セッションが生きている限り、エラーとは言わない。
      //     「つなぎ直しています」と伝えて、黙って繋ぎ直しを続ける。
      //     45秒たてば waiting に変わり、「再起動を待っています」に切り替わる。
