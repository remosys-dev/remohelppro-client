...
Future<void> prepareRebootResume() async {
  ...
  try {
    ...
    await _regHidden([
      ...
      // 🔴🔴 **黒い窓を出さない**（2026-09-02 ご指摘。実機で出続けていた）。
      //
      //   前: cmd /c if exist "…" start "" /min "…"
      //     ⚠ Windows がログオン時に **cmd.exe で実行する**ので、
      //       お客様の画面に黒い窓が出る。しかも `start /min` で2つ目も出る。
      //       `/min` は起動する側を小さくするだけで、`cmd /c` 自身は消せない。
      //   後: 署名済みの実行ファイル（窓を持たない）に走らせてもらう。
      //     ⚠ 実体は RL_RUNNER_EXE の複製＝**当社の署名入り**。
      //       中では CREATE_NO_WINDOW で命令書を呼ぶので、窓は一つも出ない。
      //   ⚠ 命令書そのものは**消さない**。20秒待って3回試して記録を残す役目は、
      //     2026-08-02 に実機で必要だと分かっている（直接呼ぶと起動しきれない）。
      '/d', '"$exePath" --rl-resume-run "$cmdPath"',
      '/f',
    ]);
