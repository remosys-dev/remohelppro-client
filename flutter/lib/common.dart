...
class MyTheme {
  ...
  // v1.4.6-13: Phase 4 - REMOHELP PRO ブランドカラー (ディープブルー)
  // RustDesk 元: 0xFF0071FF (標準ブルー)
  static const Color accent = Color(0xFF1E40AF);
  static const Color accent50 = Color(0x771E40AF);
  static const Color accent80 = Color(0xAA1E40AF);
  static const Color canvasColor = Color(0xFF212121);
  static const Color border = Color(0xFFCCCCCC);
  // v1.4.6-13: Phase 4 - ID 表示色を REMOHELP PRO シアンへ
  static const Color idColor = Color(0xFF06B6D4);
  static const Color darkGray = Color.fromARGB(255, 148, 148, 148);
  static const Color cmIdColor = Color(0xFF21790B);
  static const Color dark = Colors.black87;
  // v1.4.6-13: Phase 4 - REMOHELP PRO ボタン色 (blue-600)
...
List<String>? urlLinkToCmdArgs(Uri uri) {
  ...
  if (command != null && id != null) {
    ...
    // 接続先 (target) の CM ダイアログで client.name として表示される
    // 環境変数で Rust 側に渡す (src/client.rs で std::env::var("RL_PEER_NAME") を最優先)
    String? peerName = param["peer_name"];
    if (peerName != null && peerName.isNotEmpty) {
      bind.mainSetEnv(key: 'RL_PEER_NAME', value: peerName);
    } else {
      // 前セッション残値クリア
...
Widget loadPowered(BuildContext context) {
  ...
  return MouseRegion(
    ...
    child: GestureDetector(
      onTap: () {
        launchUrl(Uri.parse('https://remohelppro.jp'));
      },
      child: Opacity(
          opacity: 0.5,
          child: Text(
...
Widget loadIcon(double size) {
  // ワンタイム版はタイトルバー等のロゴを REMOHELP PRO チェーンリンクにし、
  // フリート版("R")と区別する。フリート版は従来どおり assets/icon.png。
  if (kRlSupportShowWindow) {
    return rlChainLogo(size);
  }
