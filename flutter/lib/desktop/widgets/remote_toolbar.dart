...
class ToolbarState {
  ...
  Future<void> init(SessionID sessionId) async {
    ...
    try {
      ...
      // 相談員ビュアー: 接続時は既定でツールバーを畳み、顧客画面を"そのまま"見せる
      //   (クリーン化)。保存値(results[0])より優先して常に畳んで開始する。
      //   復活は 上端ホバー(全画面) / ドラッグハンドル(ウィンドウ) で可能。
      collapse.value = true;
...
class _ToolbarTheme {
  ...
  // 帯を明るくしたので、ラベルは常に濃い色（テーマに依存させない）
  static Color labelColor(BuildContext context) => const Color(0xFF334155);
...
class _RemoteToolbarState extends State<RemoteToolbar> {
  ...
  // ScreenAdjustor 側にも同名 getter があるが _RemoteToolbarState からは見えない。
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      // v1.4.6-13: Phase 4 案 D - 全画面時は drag handle を出さず、上端ホバーで復活
      ...
      return Align(
        ...
        child: collapse.isFalse
            ...
            : (isFullscreen
                ? _buildEdgeHover(context)
                : _buildDraggableCollapse(context)),
      );
    });
  }

  // v1.4.6-13: Phase 4 案 D - 全画面時の上端ホバー検出 (macOS 風 auto-hide menubar)
  Widget _buildEdgeHover(BuildContext context) {
    return SizedBox(
    ...
    return Column(
      ...
      children: [
        Material(
          ...
          // REMOHELP PRO Phase2(モック準拠): 帯は常に明るい色に固定（アプリのテーマに依存させない）
  ...
  Widget _buildSessionInfoBar(BuildContext context) {
    ...
    return Obx(() {
      final viewOnly = widget.ffi.ffiModel.viewOnly;
      String conn = '確認中';
      try {
        final d = ConnectionTypeState.find(widget.id).direct.value;
        if (d == ConnectionType.strDirect) {
          conn = '直結';
        } else if (d == ConnectionType.strIndirect) {
          conn = 'リレー経由';
        }
      } catch (_) {
        /* 未初期化時は「確認中」のまま */
  ...
  Widget build(BuildContext context) {
    ...
    final icon = widget.icon ??
        SvgPicture.asset(
          widget.assetName!,
          colorFilter:
              ColorFilter.mode(_ToolbarTheme.iconOn(bgColor), BlendMode.srcIn),
          width: _ToolbarTheme.buttonSize,
          height: _ToolbarTheme.buttonSize,
        );
    // REMOHELP PRO Phase2(モック準拠): アイコンの下に日本語ラベルを表示する。
    final iconBox = Material(
        ...
        child: Ink(
            ...
            child: icon));
    final Widget content = _ToolbarTheme.showButtonLabel
        ? Column(
            mainAxisSize: MainAxisSize.min,
            ...
            children: [
              iconBox,
              SizedBox(
                height: _ToolbarTheme.labelHeight,
                child: Text(
                  translate(widget.tooltip),
                  style: TextStyle(
                      fontSize: _ToolbarTheme.labelFontSize,
                      height: 1.1,
                      color: _ToolbarTheme.labelColor(context)),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  textAlign: TextAlign.center,
                ),
    ...
    var button = SizedBox(
      width: widget.width ??
          (_ToolbarTheme.showButtonLabel
              ? _ToolbarTheme.buttonWithLabelWidth
              : _ToolbarTheme.buttonSize),
      height: _ToolbarTheme.buttonSize +
          (_ToolbarTheme.showButtonLabel ? _ToolbarTheme.labelHeight : 0),
