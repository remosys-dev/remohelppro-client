...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        ...
        if not os.path.exists("target/release/librustdesk.dll"):
            ...
            exit(-1)
    os.chdir('flutter')
    # 2026-06-23: ワンタイム・サポート用ポータブル版向けに dart-define を環境変数から追加可能に。
    #   例) RL_EXTRA_DART_DEFINE="--dart-define=RL_SUPPORT_SHOW_WINDOW=true" をCIで設定。
    #   未設定なら従来どおり(フリート版に影響なし)。
    extra_dd = os.environ.get('RL_EXTRA_DART_DEFINE', '').strip()
    system2(f'flutter build windows --release {extra_dd}'.strip())
    os.chdir('..')
